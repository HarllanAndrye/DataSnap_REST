...
type
  ...
  private
    ...
    function ExistsToken(AAuthData: String) : Boolean;
...
begin
  VHandled := AAuthType.Equals('Bearer') and ExistsToken(AAuthData);
end;

function TForm1.ExistsToken(AAuthData: String) : Boolean;
begin
  if Trim(AAuthData) <> '' then
    Result := True
  else
    Result := False;
end;
